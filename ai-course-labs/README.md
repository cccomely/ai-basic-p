...
**任务**：完成 `student_code/lab1/main.py` 中的 `classify_text` 函数

**关键要求**：
- 返回 `TextClassification` 实例（包含 category, confidence_score, keywords）
- category 必须是预定义的5个类别之一
...
**提示**：
- 在 Prompt 中明确要求模型输出 JSON 格式
- 使用 Ollama API 的 `format: "json"` 参数
- 利用 Pydantic 的自动验证功能
...
**任务**：完成 `student_code/lab2/main.py` 中的 `chat_with_memory` 函数
...
**任务**：完成 `student_code/lab3/main.py` 中的两个函数：
- `chat_with_langchain_memory`：使用 LangChain Memory 进行对话
- `get_memory_summary`：获取会话历史摘要
...
**任务**：完成 `student_code/lab4/main.py` 中的 `generate_ad` 函数
...
| 超时错误 | 模型响应慢 | 减小 max_tokens 或简化 Prompt |
